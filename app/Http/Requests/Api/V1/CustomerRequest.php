<?php

namespace App\Http\Requests\Api\V1;

use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Validation\Rule;

class CustomerRequest extends FormRequest
{
    public function authorize(): bool
    {
        return true;
    }

    public function rules(): array
    {
        $isUpdate = $this->isMethod('PUT') || $this->isMethod('PATCH');
        $partnerId = $this->route('customer') ? $this->route('customer')->id : null;

        return [
            'company_id' => ['nullable', 'integer', 'exists:companies,id'],
            'user_id' => ['nullable', 'integer', 'exists:users,id'],
            'document_type' => [$isUpdate ? 'sometimes' : 'required', Rule::in(['DNI', 'RUC', 'CE', 'Passport'])],
            'document_number' => [
                $isUpdate ? 'sometimes' : 'required',
                'string',
                'max:20',
                Rule::unique('partners')->where(function ($query) {
                    return $query->where('document_type', $this->document_type);
                })->ignore($partnerId)
            ],
            'first_name' => ['required_without:business_name', 'nullable', 'string', 'max:100'],
            'last_name' => ['required_without:business_name', 'nullable', 'string', 'max:100'],
            'business_name' => ['required_without:first_name', 'nullable', 'string', 'max:200'],
            'email' => ['nullable', 'email', 'max:100'],
            'phone' => ['nullable', 'string', 'max:20'],
            'mobile' => ['nullable', 'string', 'max:20'],
            'address' => ['nullable', 'string'],
            'district' => ['nullable', 'string', 'max:100'],
            'province' => ['nullable', 'string', 'max:100'],
            'department' => ['nullable', 'string', 'max:100'],
            'birth_date' => ['nullable', 'date'],
            'gender' => ['nullable', Rule::in(['M', 'F', 'Other'])],
            'blood_type' => ['nullable', 'string', 'max:5'],
            'medical_notes' => ['nullable', 'string'],
            'allergies' => ['nullable', 'string'],
            'status' => ['nullable', Rule::in(['active', 'inactive', 'suspended', 'blacklisted'])],
            'notes' => ['nullable', 'string'],
        ];
    }
}
